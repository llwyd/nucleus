...
{
    const bool ret = ( cyw43_wifi_link_status(&cyw43_state, CYW43_ITF_STA) == CYW43_LINK_JOIN );
    return ret;
}

extern bool WIFI_CheckTCPStatus(void)
{
    const bool ret = (cyw43_tcpip_link_status(&cyw43_state, CYW43_ITF_STA) == CYW43_LINK_UP);
    return ret;
}
